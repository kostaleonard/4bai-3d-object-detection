"""Taken from:
https://github.com/ZhixinLai/3D-detection-with-monocular-RGB-image/blob/master/math_2Dto3D.py
Some refactoring as necessary by kostaleonard."""
...
def calc_location(dimension, proj_matrix, box_2d, alpha, theta_ray):
    ...
    elif alpha < np.deg2rad(90) and alpha > -np.deg2rad(
            90):
        left_mult = -1
        right_mult = 1
...
def calc_location_new(dimension, proj_matrix, box_2d, alpha, theta_ray):
    ...
    for constraint in constraints:
        ...
        for cor_point_2d in points_2d:
            if cor_point_2d[0] < xmax + 3 and cor_point_2d[0] > xmin - 3 and \
                    cor_point_2d[1] < ymax + 3 and cor_point_2d[1] > ymin - 3:
                included += 1
        # found a better estimation
