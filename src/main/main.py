...
def build_model():
    ...
    def orientation_loss(y_true, y_pred):
        ...
        # Define the loss
        loss = (y_true[:, :, 0] * y_pred[:, :, 0] + y_true[:, :, 1] * y_pred[:,
                                                                      :, 1])
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
        ...
        dimension = slim.fully_connected(conv5, 512, activation_fn=None,
                                         scope='fc7_d')
        ...
        dimension = slim.fully_connected(dimension, 3, activation_fn=None,
                                         scope='fc8_d')
        # dimension = LeakyReLU(dimension, 0.1)
        ...
        orientation = slim.fully_connected(conv5, 256, activation_fn=None,
                                           scope='fc7_o')
        ...
        orientation = slim.fully_connected(orientation, BIN * 2,
                                           activation_fn=None, scope='fc8_o')
        # orientation = LeakyReLU(orientation, 0.1)
        ...
        confidence = slim.fully_connected(conv5, 256, activation_fn=None,
                                          scope='fc7_c')
        confidence = LeakyReLU(confidence, 0.1)
        confidence = slim.dropout(confidence, 0.5, scope='dropout7_c')
        confidence = slim.fully_connected(confidence, BIN, activation_fn=None,
                                          scope='fc8_c')
...
def train(image_dir, box2d_loc, label_dir):
    ...
    for name in range(1, len(ckpt_list), 2):
        tf.contrib.framework.init_from_checkpoint('./vgg_16.ckpt', {
            ckpt_list[name - 1][0]: variables_to_restore[name]})
        tf.contrib.framework.init_from_checkpoint('./vgg_16.ckpt', {
            ckpt_list[name][0]: variables_to_restore[name - 1]})
