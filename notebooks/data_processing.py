...
def parse_annotation_s3(filenames, s3_label_dir):
    """Leo's note: returns as a dict all of the vehicle labels for vehicles that
    are not truncated or occluded."""
    all_objs = []
    dims_avg = {key: np.array([0, 0, 0]) for key in VEHICLES}
    ...
    print('Reading labels from S3.')
    for image_file in tqdm(filenames):
        label_file = image_file.replace('.png', '.txt')
        with fs.open(os.path.join(s3_label_dir, label_file)) as infile:
            for line in infile.readlines():
...
def prepare_input_and_output(image_dir, train_inst):
    ...
    if flip > 0.5:
        return img, train_inst['dims'], train_inst['orient_flipped'], \
               train_inst['conf_flipped']
    else:
        return img, train_inst['dims'], train_inst['orient'], train_inst['conf']

    
def prepare_input_and_output_s3(s3_image_dir, train_inst):
    """Leo's note: returns the image, dimensions, orientation, and confidence
    associated with a given object in a training image. This function cuts out
    the object from the image, resizes to 224x224, possibly flips the image
    horizontally (50% chance), subtracts out the mean RGB values, and returns
...
def data_gen_s3(s3_image_dir, all_objs, batch_size):
    """Leo's note: generates batches of x, y pairs, where x is the image data
    and y is a 3-tuple of (dimension, orientation, confidence)."""
    num_obj = len(all_objs)
    ...
    while True:
        ...
        for key in keys[l_bound:r_bound]:
            # augment input image and fix object's orientation and confidence
            image, dimension, orientation, confidence = prepare_input_and_output_s3(
                s3_image_dir, all_objs[key])
